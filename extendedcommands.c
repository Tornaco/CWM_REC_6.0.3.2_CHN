...
{
    static char* headers[] = {  "从sd卡选择zip包并刷入",
                                "",
                                NULL
    };
    
    char* install_menu_items[] = {  "手动选择SD卡中的刷机包",
                                    "自动刷入SD卡中update.zip包-如果有该包的话",
                                    "开启/关闭刷写文件验证",
                                    NULL,
                                    NULL };
    ...
    if (volume_for_path("/emmc") != NULL) {
        other_sd = "/emmc/";
        install_menu_items[3] = "从内置SD卡中选择刷机包";
    }
    else if (volume_for_path("/external_sd") != NULL) {
        other_sd = "/external_sd/";
        install_menu_items[3] = "从外置SD卡中选择刷机包";
    }
...
void show_choose_zip_menu(const char *mount_point)
{
    ...
    sprintf(confirm, "安装 %s", basename(file));
...
void show_nandroid_restore_menu(const char* path)
{
    ...
    static char* headers[] = {  "选择一个镜像进行恢复",
                                "",
                                NULL
    };
...
int format_unknown_device(const char *device, const char* path, const char *fs_type)
{
    ...
    if (0 == strcmp(path, "/sd-ext"))
    {
        ...
        {
            ui_print("没找到app2sd分区，所以不再格式化/sd-ext.\n");
            return 0;
        }
    }
...
void show_nandroid_advanced_restore_menu(const char* path)
{
    ...
    static char* advancedheaders[] = {  "选择一个镜像进行恢复",
                                "",
                                "选择一个镜像进行恢复",
                                "下一个菜单将会",
                                "提供给你更多信息.",
    ...
    {
        case 0:
            if (confirm_selection(confirm_restore, "好的，恢复boot分区"))
                nandroid_restore(file, 1, 0, 0, 0, 0, 0);
            break;
        case 1:
            if (confirm_selection(confirm_restore, "好的，恢复system分区"))
                nandroid_restore(file, 0, 1, 0, 0, 0, 0);
            break;
        case 2:
            if (confirm_selection(confirm_restore, "好的，恢复data分区"))
                nandroid_restore(file, 0, 0, 1, 0, 0, 0);
            break;
        case 3:
            if (confirm_selection(confirm_restore, "好的，恢复缓存cache分区"))
                nandroid_restore(file, 0, 0, 0, 1, 0, 0);
            break;
        case 4:
            if (confirm_selection(confirm_restore, "好的，恢复sd-ext分区"))
                nandroid_restore(file, 0, 0, 0, 0, 1, 0);
            break;
        case 5:
            if (confirm_selection(confirm_restore, "好的，恢复wimax分区"))
...
static void partition_sdcard(const char* volume) {
    ...
    static char* ext_headers[] = { "Ext 分区大小", "", NULL };
    static char* swap_headers[] = { "Swap 分区大小", "", NULL };
...
void handle_failure(int ret)
{
    ...
    ui_print("报告文件位于/tmp/recovery.log was copied to /sdcard/clockworkmod/recovery.log. 请用ROM Manager或者微博@guohow报告给我们.\n");
}
...
int verify_root_and_recovery() {
    ...
    if (0 == lstat("/system/etc/install-recovery.sh", &st)) {
        if (st.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH)) {
            ui_show_text(1);
            ret = 1;
            if (confirm_selection("ROM重启可能会自动恢复官方recvery帮你阻止？", "帮帮我")) {
    ...
    if (0 == lstat("/system/bin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            if ((st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_show_text(1);
                ret = 1;
                if (confirm_selection("可能会丢失ROOT权限. 防止丢失?", "帮帮我(/system/bin/su)")) {
    ...
    if (0 == lstat("/system/xbin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            if ((st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_show_text(1);
                ret = 1;
                if (confirm_selection("可能会丢失ROOT权限. 防止丢失?", "帮帮我( (/system/xbin/su)")) {
    ...
    if (!exists) {
        ui_show_text(1);
        ret = 1;
        if (confirm_selection("可能您没有ROOT，帮我ROOT?", "帮帮我 (/system/xbin/su)")) {
