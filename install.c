...
try_update_binary(const char *path, ZipArchive *zip) {
    ...
    if (binary_entry == NULL) {
        ...
        if (update_script_entry != NULL) {
            ui_print("Amend scripting (update-script) 不再被支持.\n");
            ui_print("Amend scripting 在android1.5 被谷歌废弃.\n");
            ui_print("It was necessary to remove it when upgrading to the ClockworkMod 3.0 Gingerbread based recovery.\n");
            ui_print("请切换ROM到Edify scripting (updater-script and update-binary) 脚本.\n");
...
really_install_package(const char *path)
{
    ...
    if (signature_check_enabled) {
        ...
        if (err != VERIFY_SUCCESS) {
            ...
            ui_show_text(1);
            if (!confirm_selection("安装不可信的刷机包吗？", "安装吧..."))
                return INSTALL_CORRUPT;
        }
    }
